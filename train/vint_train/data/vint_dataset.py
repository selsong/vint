...
class ViNT_Dataset(Dataset):
    ...
    def _build_caches(self, use_tqdm: bool = True):
        """
        Build a cache of video frames for faster loading using LMDB
        """
        cache_filename = os.path.join(
            self.data_split_folder,
            f"dataset_{self.dataset_name}_video.lmdb",
        )
        ...
        """
        If the cache file doesn't exist, create it by extracting frames from videos and writing to the cache
        """
        if not os.path.exists(cache_filename):
            ...
            with lmdb.open(cache_filename, map_size=2**40) as frame_cache:
                with frame_cache.begin(write=True) as txn:
                    processed_videos = set()
                    
                    for traj_name, time in tqdm_iterator:
                        # Only process each video once
                        if traj_name in processed_videos:
                            continue
                            
                        processed_videos.add(traj_name)
                        video_path = os.path.join(self.data_folder, traj_name, "video.mp4")
                        ...
                        for frame_idx in range(frame_count):
                            ...
                            # Create a key for the frame: traj_name + frame_idx
        ...
        self._frame_cache = lmdb.open(cache_filename, readonly=True)
